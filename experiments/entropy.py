...
samplers = ['random', 'modal', 'alipy', 'libact', 'cardinal']
...
for seed, dataset, classifier, sampler in itertools.product(seeds, datasets, classifiers, samplers):
    ...
    methods = {
        ...
        'modal': ModalWrapper(ActiveLearner(clf, entropy_sampling), batch_size=batch_size, refit=False),
        'alipy': AlipyWrapper(clf, lambda x,y: QueryInstanceUncertainty(x, y, measure='entropy'), batch_size=batch_size),
        'libact': LibactWrapper(clf, UncertaintySampling, batch_size=batch_size, init_params={'method': 'entropy'}),
