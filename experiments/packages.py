...
samplers = ['random', 'modal', 'alipy', 'libact']
...
for seed, dataset, classifier, sampler in itertools.product(seeds, datasets, classifiers, samplers):
    ...
    methods = {
        ...
        'modal': ModalWrapper(clf, uncertainty_sampling, batch_size=batch_size),
...
for dataset, classifier in itertools.product(datasets, classifiers):
    ...
    for sampler in samplers:
        ...
        data = np.vstack(all_results[(dataset, classifier, sampler)][1])
